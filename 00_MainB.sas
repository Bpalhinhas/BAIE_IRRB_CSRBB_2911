...
%CURVAS_CSRBB(AUX.TABELA_CURVAS_CSRBB);
...
%reconciliacao(DQ, out.TABELA_MASTER_FINAL, AUX.BALANCETE_AGREG, '30JUN2024'd);

/*%Totais_Carteiras(work,  AUX.BALANCETE_AGREG, MONTANTE_CAPITAL, B_RecDO_EUR, MOEDA, 'EUR', ID, in ('400010','400020','40010'));
/*Tratamento das curvas do Indexante*/
...
/*Tabela Master + Nova produção / calculo do NII*/

/*%BALANCO_ESTATICO(OUT.TABELA_MASTER_FINAL, OUT.NOVA_PRODUCAO, OUT.BALANCO_ESTATICO);*/


%PROJ_NP_DESCONTO(AUX.TABELA_CURVAS_INDEXANTE,AUX.ALOCACAO_BUCKET, OUT.CASHFLOWS_DESCONTADOS_NII);

/************************************************************************************************/
/*****************************************Calculo CSRBB*****************************************/

/*Seleção do universo elegível:
		Obrigações com nível de FAIR_VALUE inferior ou igual  2 */

%UniversoCSRBB_EVE(AUX.ALOCACAO_BUCKET, AUX.TABELA_CURVAS_CSRBB, AUX.INPUT_CSRBB_);

/*Calculo do VaR*/

/*EVE*/

%CalculoEVE_CSRBB(AUX.INPUT_CSRBB_, AUX.TABELA_CURVAS_INDEXANTE, OUT.EVE_CSRBB);


/*Seleção do universo elegível NII NP:
		Obrigações com nível de FAIR_VALUE inferior ou igual  2 */
%UniversoCSRBB_NII(OUT.NOVA_PROD, AUX.TABELA_CURVAS_CSRBB,AUX.INPUT_CSRBB_NII);

/*NII*/
%CalculoNII_CSRBB(AUX.INPUT_CSRBB_NII, AUX.TABELA_CURVAS_INDEXANTE, OUT.NII_CSRBB);